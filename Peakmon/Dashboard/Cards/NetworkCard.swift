...
//  Peakmon
//
//  Popover network card: down/up rates, combined throughput
...
struct NetworkCard: View {
    ...
    var body: some View {
        DashboardCardTemplate(
            ...
            tint: tint,
            stats: [
                CardStat(label: "Down", value: DashboardFormatting.rateShort(netIn), tint: .green),
                CardStat(label: "Up", value: DashboardFormatting.rateShort(netOut), tint: .pink),
            ],
            accessory: {
                let total = netIn + netOut
                Text(DashboardFormatting.rateHeadline(total))
