...
//  (visibility, tint, and order) so call sites no longer hand-roll one
//  `@CardVisibilityStorage` / `@CardTintStorage`
//  per slot per view.
//
...
//  keep the property wrappers in a single container view —
...
//
//  Read access goes through `settings.visibility(slot) / tint(slot)`;
//  write access through the corresponding bindings so SwiftUI controls
//  (`Toggle`, `ColorPicker`) keep working.
//
...
/// Holds the per-slot `@CardXxxStorage` wrappers in a single view and
/// injects a `CardSettings` into its subtree via the environment.
///
