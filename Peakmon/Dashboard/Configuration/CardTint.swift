...
enum CardTintSlot: String, CaseIterable, Identifiable, Codable, Equatable, Transferable {
    ...
    /// Factory order used until the user customises the Popover in
    /// Display settings.
    static let displayOrder: [CardTintSlot] = [
        .cpu, .memory, .gpu, .power, .disk, .network, .battery, .processes,
    ]
    ...
    /// Factory default for the visibility flag. Processes now forms
    /// the final dashboard section; its collector is still gated by
    /// main-window visibility so it sleeps when the dashboard does.
