...
//  Peakmon
//
//  Lays out visible dashboard cards into rows that honour the user's
//  per-card width preference (`CardWidth`). Two consecutive half-width
//  cards in the user's chosen visible order pack into one HStack row;
//  full-width cards always occupy a row alone; an unpaired trailing
//  half-width card is rendered at half width with a leading empty
//  spacer so the layout does not jitter when only one half card is
//  visible.
//
...
/// (visibility, width) for the visible slots in declaration order;
